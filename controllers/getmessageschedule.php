<?php
    ...
    if ($GLOBALS['user']->Id != "")
    {
        if($GLOBALS['user']->Role->Booking->ReadAccess)
        {
            ...
            {
                $store = Messageschedule::SystemEvents($GLOBALS['subscriber']);
                $ret->Completed = Messageschedule::Completedsystemschedule($GLOBALS['subscriber']);
            }
            else
            {
                $store = Messageschedule::UserEvents($GLOBALS['subscriber']);
                $ret->Completed = Messageschedule::Completeduserchedule($GLOBALS['subscriber']);
            }
