        ...
        if ($GLOBALS['user']->Role->Customers->ReadAccess)
        {
            ...
            if ($_REQUEST['tab'] == "all")
            {
                ...
                else
                {
                    ...
                    //$ref = new ReflectionClass(Lodging::class);

                    //var_dump($ref->getFileName()); die;

                }
            }
            elseif ($_REQUEST['tab'] == "due-check-out")
            {
                $store = Lodging::dueToday(new Subscriber($property->Databasename, $property->DatabaseUser, $property->DatabasePassword));
            }
            elseif ($_REQUEST['tab'] == "overdue-check-out")
            {
                $store = Lodging::overdue(new Subscriber($property->Databasename, $property->DatabaseUser, $property->DatabasePassword));
            }

            $ret->inhouse = count(Lodging::All(new Subscriber($property->Databasename, $property->DatabaseUser, $property->DatabasePassword), true));
            ...
            $ret->overdue = Lodging::overdueCount(new Subscriber);
            $ret->status = "success";
