	...
	class CustomerByProperty
	{
		...
		public function Initialize($arg=null)
        {
            if($arg != null)
            {
                ...
                $res = $db->query("SELECT * FROM customerByProperty WHERE customerid='$arg' AND propertyid = '$property'");
                ...
                if($res->num_rows > 0)
                {
                    ...
                    $this->Zip = $row['zip'];
                    $this->Lastseen = self::getLastCheckedOut($row['customerid']);
		...
		public function Save()
		{
			...
			if($id != "" && $db->query("SELECT customerid FROM customerByProperty WHERE customerid='$id' AND propertyid = '$property'")->num_rows > 0)
			{
			...
			else
			{
                ...
                {
                    if ($db->query("SELECT customerid FROM customer WHERE customerid='{$this->Id}'")->num_rows == 0)
                    {
                        $canAdd = true;
                    }
                }
                ...
                if (true)
                {
                    ...
                    $template->Initialize('6jjqk6494zb4r5si');
                    $mail->Body = $template->Body;

                    // add full name
                    $mail->Body = str_replace('{contactperson}', ucwords($name . ' ' . $surname), $mail->Body);

                    // get url
                    $url = rtrim(Configuration::url()->host, '/') . '/complete-setup/' . $id;

                    // build link
                    $link = '<a href="'.$url.'">Complete profile setup</a>';

                    // add link
                    $mail->Body = str_replace('{link}', $link, $mail->Body);

                    // replace host
                    $mail->Body = str_replace('{host}', '<a href="'.rtrim(Configuration::url()->host, '/').'">TripMata.com</a>', $mail->Body);
                    ...
                    $mail->isHTML = 'true';
                    $mail->To = $email;
                    $mail->ToName = ucwords($name . ' ' . $surname);
        ...
        private static function getLastCheckedOut(string $customerId)
        {
            $db = DB::GetDB();
            $property = $_REQUEST['property'];
            $lastseen = 0;
            $query = $db->query("SELECT checkout FROM lodging WHERE guest = '$customerId' AND propertyid = '$property' AND checkedout = 1 ORDER BY id DESC LIMIT 0,1");
            ...
            if ($query->num_rows > 0)
            {
                ...
                // get last seen
                $lastseen = new WixDate($data['checkout']);
            }
		...
		public static function Search(Subscriber $subscriber, $term='')
		{
            ...
            while(($row = $res->fetch_assoc()) != null)
			{
                if (self::customerExists($row['customerid'])) :
                    ...
                    $ret[$i]->Zip = $row['zip'];
                    $ret[$i]->Lastseen = self::getLastCheckedOut($row['customerid']);
        ...
        public static function PhoneExist($phone, Subscriber $sub)
        {
            $db = $sub->GetDB();
        ...
        {
            $db = $sub->GetDB();
            return $db->query("SELECT guestid FROM customerByProperty WHERE guestid='$guestid'")->num_rows > 0 ? true : false;
        }
        ...
        public static function ByEmail(Subscriber $sub, $email)
        {
            $email = strtolower(trim($email));
            $db = $sub->GetDB();

            $ret = new CustomerByProperty($sub);
